...
export class ServicePlanValidationService {
  ...
  validateAllForms(): Map<number, IStepValidationErrors> {
    ...
    // Validate Step 1 - Cover Page
    const step1Errors = this.validateFormGroup(
      this.formService.step1_coverPage,
      'Step 1'
    );
    errors.set(1, {
      stepNumber: 1,
    ...
    // Validate Step 2 - Overview
    const step2Errors = this.validateFormGroup(
      this.formService.step2_overview,
      'Step 2'
    );
    errors.set(2, {
      stepNumber: 2,
    ...
    // Validate Step 3 - Existing Saudi Co.
    const step3Errors = this.validateFormGroup(
      this.formService.step3_existingSaudi,
      'Step 3'
    );
    errors.set(3, {
      stepNumber: 3,
    ...
    // Validate Step 4 - Direct Localization
    const step4Errors = this.validateFormGroup(
      this.formService.step4_directLocalization,
      'Step 4'
    );
    errors.set(4, {
      stepNumber: 4,
  ...
  markAllFieldsAsTouched(): void {
    this.markFormGroupAsTouched(this.formService.step1_coverPage);
    this.markFormGroupAsTouched(this.formService.step2_overview);
    this.markFormGroupAsTouched(this.formService.step3_existingSaudi);
    this.markFormGroupAsTouched(this.formService.step4_directLocalization);
  }
