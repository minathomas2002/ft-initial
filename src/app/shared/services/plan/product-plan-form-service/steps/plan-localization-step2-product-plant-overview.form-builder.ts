...
export class PlanLocalizationStep2ProductPlantOverviewFormBuilder extends BasicPlanBuilder {
  ...
  toggleSECFieldsValidation(formGroup: FormGroup, provideToSEC: boolean): void {
    ...
    if (provideToSEC) {
      ...
      experienceFormGroup.get(`${EMaterialsFormControls.totalQuantitiesSEC}.${EMaterialsFormControls.value}`)?.setValidators([Validators.min(0), Validators.required, Validators.max(1000000000000000)]);
    } else {
  ...
  toggleLocalSuppliersFieldsValidation(formGroup: FormGroup, provideToLocalSuppliers: boolean): void {
    ...
    if (provideToLocalSuppliers) {
      ...
      experienceFormGroup.get(`${EMaterialsFormControls.totalQuantities}.${EMaterialsFormControls.value}`)?.setValidators([Validators.min(0), Validators.required, Validators.max(1000000000000000)]);
    } else {
