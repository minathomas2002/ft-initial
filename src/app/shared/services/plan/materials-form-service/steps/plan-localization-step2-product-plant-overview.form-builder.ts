...
export class PlanLocalizationStep2ProductPlantOverviewFormBuilder {
  ...
  toggleSECFieldsValidation(formGroup: FormGroup, provideToSEC: boolean): void {
    ...
    if (provideToSEC) {
      ...
      experienceFormGroup.get(`${EMaterialsFormControls.totalQuantitiesSEC}.${EMaterialsFormControls.value}`)?.setValidators([Validators.required, Validators.min(1), Validators.max(10000000000)]);
    } else {
  ...
  toggleLocalSuppliersFieldsValidation(formGroup: FormGroup, provideToLocalSuppliers: boolean): void {
    ...
    if (provideToLocalSuppliers) {
      ...
      experienceFormGroup.get(`${EMaterialsFormControls.totalQuantities}.${EMaterialsFormControls.value}`)?.setValidators([Validators.required, Validators.min(1), Validators.max(10000000000)]);
    } else {
